...
import zan_y from "../../assets/zan_y.png";

import defaults from "../../assets/default.png";
import { UserStorage } from "src/storage/UserStorage";

interface QuestionDetailsOtherProps {
  history: History;
  location: Location;
}

interface QuestionDetailsOtherState {
  title: string;
  orderid: string;
  isHome: boolean;
  status: string;
...
export class QuestionDetailsOther extends React.Component<QuestionDetailsOtherProps, QuestionDetailsOtherState> {
  lv: any;
  orderid: any;
  isStatus: boolean;
  ...
  description: string;
  data:any;
  constructor(props: QuestionDetailsOtherProps) {
    super(props);
    this.isStatus= false;
  ...
  componentDidMount() {
    let cookie = false;
      if(UserStorage.getCookie('orderid')){
        cookie = true
      }
    //价格
      ...
      <div className="message-container">
        ...
        >
          ...
          <div className="wpadding">
            ...
            <div className="order_label c999 flex">
              订单：{this.state.orderid}
              </div>
              <span className="fs14 padding-l1">{this.state.answered_number}/{this.state.answer_number}个回答</span>
            </div>
            <div className="fs15 lh14 wwb padding-tb">
              问题：<span className="fs18 c000 fb">{this.state.title}</span>
            </div>
                  ...
                  <div key={index} className="runAnwser">
                    ...
                    <div className="flex c999 flex-j-end">
                      <div className={"w1 padding-l45 "+ (data.run_ask.length < 1?'':'none')}>
                        <Button type={"ghost"} size={"small"} data-id={data.id} data-issueid={data.issueid} onClick={this.questioning}>追问</Button>
                      </div>
                      <div className="flex set_star w40 padding-l2">
                        <img src={data.answer_star > '0'?credit:this.state.answer_star > '0' && this.state.id == data.id?credit:credit_n} data-star={'1'} data-answerid={data.id} data-answerstar={data.answer_star} onClick={this.setStar}/>
                        <img src={data.answer_star > '1'?credit:this.state.answer_star > '1' && this.state.id == data.id?credit:credit_n} data-star={'2'} data-answerid={data.id} data-answerstar={data.answer_star} onClick={this.setStar}/>
                        <img src={data.answer_star > '2'?credit:this.state.answer_star > '2' && this.state.id == data.id?credit:credit_n} data-star={'3'} data-answerid={data.id} data-answerstar={data.answer_star} onClick={this.setStar}/>
                        <img src={data.answer_star > '3'?credit:this.state.answer_star > '3' && this.state.id == data.id?credit:credit_n} data-star={'4'} data-answerid={data.id} data-answerstar={data.answer_star} onClick={this.setStar}/>
                        <img src={data.answer_star > '4'?credit:this.state.answer_star > '4' && this.state.id == data.id?credit:credit_n} data-star={'5'} data-answerid={data.id} data-answerstar={data.answer_star} onClick={this.setStar}/>
                      </div>
                      <div className="w25 tar" data-id={data.id} onClick={this.setFabulous}>
                        <img className="content_img margin-rs" src={data.fabul_status == '0'?zan:zan_y} alt="" />
                    ...
                    <div className={this.state.showQuestioning && this.state.id == data.id?'mbp':'none'}>
