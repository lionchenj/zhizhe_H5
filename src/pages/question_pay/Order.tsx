...
//支付
  gopay = () => {
    ...
    UIUtil.goPay(this, this.state.orderid, this.cardid, type);
  }

//优惠券
getCoupon = () => {
  this.props.history.push({pathname:"/couponType",state:{data:this.data,couponList:this.state.couponList,push:'/order'}});
}
  public componentDidMount() {
    ...
    price = data.answer_number*data.answer_price,
    coupontype = '';
    ...
    if(data.type == '2'){
      ...
      coupontype = '2';
      this.setState({
        payPrice:data.answer_number*data.answer_price,
        balance:this.props.location.state.couponP||0,
        coupontype:'2',
        question_type: data.question_type,
        hotQue: data.questionid,
        isShow:true,
        orderid:data.orderid,
        price:data.price,
        answer_number: data.answer_number,
        answer_price: data.answer_price,
        type:'2'
      })
    }else{
      coupontype = '1';
    ...
    UserService.Instance.myCardCount(price,coupontype).then( (res:any) => {
      this.setState({
  ...
  public render() {
    return (
      <div className="withdraw-container order_pay">
        ...
        <List>
          <WingBlank className={this.state.isShow?"none":"padding-tb fs17"}>
          ...
          <WingBlank className={this.state.isShow?"padding-tb fs17":"none"}>
            <div className="flex">
              <div className="c999 w7 padding-b">话题：</div>
              <div>{this.state.question_type}</div>
            </div>
              {
                this.state.hotQue.map((data:any,index:string)=>{
                  return (<div className="flex flex-a-start">
                    <div className="c999 w7">{"问题"+(index+1)+"："}</div>
                    <div>
        ...
        <div className="order_foot flex">
          ...
          <div className="order_foot_btn back329" onClick={this.gopay}>{this.isType?"去支付":"确认提问"}</div>
        </div>        
      </div>
    );
  }
